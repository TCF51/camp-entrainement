...
// Met a jour la consigne d'un exercice au sein d'un camp : reserve au createur du camp
// (la "casquette entraineur"). Les autres membres ne peuvent que consulter.
router.put("/", async (req: AuthRequest, res) => {
  ...
  if (camp.createdById !== req.userId) {
    return res.status(403).json({ error: "Seul le createur du camp peut definir les consignes des exercices." });
  }
